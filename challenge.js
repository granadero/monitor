...
class HealthMonitor {
  ...
  // eg. checkRedisClient connects to redis and executes a trivial query.
  ...
  async check() {
    ...
    for (const pair of this.monitors) {
      const checkFunc = handlerFuncs[pair[0]]; //pair[0]=== type example: serviceUrl
      if (checkFunc) {
        for (const resources of pair[1]) { //pair[1] === name example: gaphql
          healthPromises.push(checkFunc(resources[0], resources[1]));
        }
      }
    }
